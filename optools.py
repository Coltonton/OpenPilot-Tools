...
class ToolUtility:
    def __init__(self):              #Init
        while True:
            util_options = ['IP Configuration (IPV4)', 'Configure DNS', 'Cleanup for uninstall', '-Reboot-', '-Quit-']
            ...
            if   selected_util == 'IP Configuration (IPV4)':
                self.IPV4_Config()
            elif selected_util == 'Configure DNS':
                self.DNS_Config()
    ...
    def IPV4_Config(self):
        ...
        if indexChoice:                                                               #If user selection is a valid program selection
            ...
            connection_to_edit = connectiondata["selected_connection"]                     #Set the Connection_to_edit var based on the corrosponding all_connections index
            ...
            if indexChoice2:
                if(ip_mode=="Static"):                                                   #If user wants to set Static IP
                    ...
                    user_subnet  = input(f"Enter Subnet or ENTER for [{"255.255.255.0"}]: ") or "255.255.255.0"  #Get desired network Subnet
                    user_gateway = input(f"Enter Gateway (Router) or ENTER for [{"192.168.1.1"}]: ") or "192.168.1.1" #Get desired network Gateway
                    generated_ip_cidr = get_cidr(user_ip, user_subnet)                                             #Generate IP in CIDR format
